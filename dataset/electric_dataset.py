...
class ElectricDataset(data.Dataset):
    def __init__(self, preprocessor):
        ...
        df1 = pd.read_csv(const.SUPPLY_DEMAND_ELECTRICITY_FILES[0])
        df2 = pd.read_csv(const.SUPPLY_DEMAND_ELECTRICITY_FILES[1])
        df = pd.concat([df1, df2], axis=0).drop_duplicates().drop(columns=['日期', '備轉容量率(%)'])
        y = preprocessor.preprocessing(np.expand_dims(df['備轉容量(MW)'].to_numpy(), axis=1), 'y')
        # x = preprocessor.preprocessing(np.expand_dims(df['備轉容量(MW)'].to_numpy(), axis=1), 'x')
